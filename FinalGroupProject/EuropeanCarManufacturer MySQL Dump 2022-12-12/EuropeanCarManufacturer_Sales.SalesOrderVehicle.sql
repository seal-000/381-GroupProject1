...
CREATE TABLE `Sales.SalesOrderVehicle` (
  ...
  `InvoiceNumber` char(8) DEFAULT NULL,
  `TotalSalePrice` decimal(18,2) DEFAULT NULL,
  `SaleDate` date DEFAULT NULL,
  PRIMARY KEY (`SalesOrderVehicleId`),
  KEY `FK_SalesOrderVehicle_Customer` (`CustomerId`),
  KEY `FK_SalesOrderVehicle_Staff` (`StaffId`),
  CONSTRAINT `FK_SalesOrderVehicle_Customer` FOREIGN KEY (`CustomerId`) REFERENCES `Triggered.Customer` (`CustomerId`),
  CONSTRAINT `FK_SalesOrderVehicle_Staff` FOREIGN KEY (`StaffId`) REFERENCES `Triggered.Staff` (`StaffId`)
